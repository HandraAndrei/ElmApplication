...
compare (Interval intA) (Interval intB) =
        let
            equalStart int1 int2 =
                case (int1.end, int2.end) of
                   (Nothing, Nothing) -> EQ
                   (Nothing, Just _) -> GT
                   (Just _, Nothing) -> LT
                   (Just _, Just _) -> if (Basics.compare (Maybe.withDefault (0,0) (length (Interval int1))) (Maybe.withDefault (0,0) (length (Interval int2))) == LT) then
                                                LT
                                           else if (Basics.compare (Maybe.withDefault (0,0) (length (Interval int1))) (Maybe.withDefault (0,0) (length (Interval int2))) == GT) then
                                                    GT
                                                else
                                                        EQ
        in
...
view interval =
    let
        ...
        endDateMissing data3 = case data3 of
                                Nothing -> text "Present"
                                Just d3 -> Date.view d3


        intervalLength (Interval interval1) = if (Maybe.withDefault (0,0) (length (Interval interval1)) == (0,0)) then
                                        p[][]
                                   else
                                        p[class "interval-length"] [text ("Years: " ++ String.fromInt (Tuple.first(Maybe.withDefault (0,0) (length (Interval interval1))))
                                        ++ " Months: " ++ String.fromInt (Tuple.second(Maybe.withDefault (0,0) (length (Interval interval1)))))]
    in
    div [class "interval"] 
    [ p [class "interval-start"] [Date.view start]
    , p [class "interval-end"] [endDateMissing end]
